...
class Amarelinha:
    def __init__(
        ...
        self.calibration_mode = CalibrationMode.AUTO
        self.settings_error = ""
    ...
    def run(self) -> None:
        ...
        while running:
            ...
            if self.screen_state == ScreenState.CALIBRATION:
                self.camera.update()
                if self.calibration_mode == CalibrationMode.AUTO:
    ...
    def _handle_key(self, key: int, modifiers: int) -> bool:
        ...
        if control_pressed and key in (pygame.K_c, pygame.K_m, pygame.K_d):
            if key == pygame.K_c:
    ...
    def _start_manual_calibration(self) -> None:
        self.camera.ensure_devices()
        self.calibration_mode = CalibrationMode.MANUAL
        self.calibration.clear()
        self.calibration_message = (
            "Clique no preview: superior esquerdo, superior direito, "
    ...
    def _handle_calibration_key(self, key: int) -> bool:
        if key == pygame.K_ESCAPE:
            ...
            and self.calibration.complete
        ):
            self.calibration.points = self._manual_points_to_projection_order()
            ...
            self.screen_state = ScreenState.GAME
        elif self.calibration_mode == CalibrationMode.MANUAL and key == pygame.K_BACKSPACE:
    ...
    def _handle_calibration_click(self, position: tuple[int, int]) -> None:
        if self.calibration_mode != CalibrationMode.MANUAL:
            return

        point = self.renderer.calibration_point_at(position)
        if point is None:
            self.calibration_message = "Clique dentro do preview da câmera."
            return
    ...
    def _set_calibration_instruction(self) -> None:
        if self.calibration.complete:
            self.calibration_message = "Enter salva. Backspace desfaz. R reinicia."
            return
        ...
        self.calibration_message = f"Clique no canto {labels[len(self.calibration.points)]}."

    def _manual_points_to_projection_order(self) -> list[list[float]]:
        preview_top_left, preview_top_right, preview_bottom_left, preview_bottom_right = (
            self.calibration.points
        )
        return [
